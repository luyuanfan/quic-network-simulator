...
NS_LOG_COMPONENT_DEFINE("ns3 fat tree simulator");
...
int main(int argc, char *argv[])
{
  ...
  Config::SetDefault("ns3::Ipv4GlobalRouting::RandomEcmpRouting", BooleanValue(ECMProuting));
	...
	uint32_t queuePkts = std::stoul(queue);
	...
	hostToEdge.SetDeviceAttribute ("DataRate", StringValue(bandwidth));
	hostToEdge.SetChannelAttribute ("Delay", StringValue(delay));
	// hostToEdge.SetQueueSize(StringValue(std::to_string(leafQSize) + "p"));
	// hostToEdge.SetQueue("ns3::DropTailQueue", "MaxSize",
	// 								StringValue(std::to_string(leafQSize) + "p"));
	
	PointToPointHelper edgeToAggr; 
	edgeToAggr.SetDeviceAttribute ("DataRate", StringValue(bandwidth));
	edgeToAggr.SetChannelAttribute ("Delay", StringValue(delay));
	// edgeToAggr.SetQueueSize(StringValue(std::to_string(uplinkQSize) + "p")); 
	// edgeToAggr.SetQueue("ns3::DropTailQueue", "MaxSize",
	// 							StringValue(std::to_string(uplinkQSize) + "p"));
	
	PointToPointHelper aggrToCore;
	aggrToCore.SetDeviceAttribute ("DataRate", StringValue(bandwidth));
	aggrToCore.SetChannelAttribute ("Delay", StringValue(delay));
	// aggrToCore.SetQueueSize(StringValue(std::to_string(uplinkQSize) + "p")); 
	// aggrToCore.SetQueue("ns3::DropTailQueue", "MaxSize",
	// 							StringValue(std::to_string(uplinkQSize) + "p"));
	...
	pod0_dev = hostToEdge.Install( pod0.Get(0), pod0.Get(4));
	pod0_dev2 = hostToEdge.Install( pod0.Get(1), pod0.Get(4));
	pod0_dev3 = hostToEdge.Install( pod0.Get(2), pod0.Get(5));
	pod0_dev4 = hostToEdge.Install( pod0.Get(3), pod0.Get(5));
	pod0_dev5 = edgeToAggr.Install( pod0.Get(4), pod0.Get(6));
	pod0_dev6 = aggrToCore.Install( pod0.Get(4), pod0.Get(7));
	pod0_dev7 = edgeToAggr.Install( pod0.Get(5), pod0.Get(6));
	pod0_dev8 = aggrToCore.Install( pod0.Get(5), pod0.Get(7));
	
	NetDeviceContainer pod1_dev,pod1_dev2,pod1_dev3,pod1_dev4,pod1_dev5,pod1_dev6,pod1_dev7,pod1_dev8;
	pod1_dev = hostToEdge.Install( pod1.Get(0), pod1.Get(4));
	pod1_dev2 = hostToEdge.Install( pod1.Get(1), pod1.Get(4));
	pod1_dev3 = hostToEdge.Install( pod1.Get(2), pod1.Get(5));
	pod1_dev4 = hostToEdge.Install( pod1.Get(3), pod1.Get(5));
	pod1_dev5 = edgeToAggr.Install( pod1.Get(4), pod1.Get(6));
	pod1_dev6 = aggrToCore.Install( pod1.Get(4), pod1.Get(7));
	pod1_dev7 = edgeToAggr.Install( pod1.Get(5), pod1.Get(6));
	pod1_dev8 = aggrToCore.Install( pod1.Get(5), pod1.Get(7));
	
	NetDeviceContainer pod2_dev,pod2_dev2,pod2_dev3,pod2_dev4,pod2_dev5,pod2_dev6,pod2_dev7,pod2_dev8;
	pod2_dev = hostToEdge.Install( pod2.Get(0), pod2.Get(4));
	pod2_dev2 = hostToEdge.Install( pod2.Get(1), pod2.Get(4));
	pod2_dev3 = hostToEdge.Install( pod2.Get(2), pod2.Get(5));
	pod2_dev4 = hostToEdge.Install( pod2.Get(3), pod2.Get(5));
	pod2_dev5 = edgeToAggr.Install( pod2.Get(4), pod2.Get(6));
	pod2_dev6 = aggrToCore.Install( pod2.Get(4), pod2.Get(7));
	pod2_dev7 = edgeToAggr.Install( pod2.Get(5), pod2.Get(6));
	pod2_dev8 = aggrToCore.Install( pod2.Get(5), pod2.Get(7));
	
	NetDeviceContainer pod3_dev,pod3_dev2,pod3_dev3,pod3_dev4,pod3_dev5,pod3_dev6,pod3_dev7,pod3_dev8;
	pod3_dev = hostToEdge.Install( pod3.Get(0), pod3.Get(4));
	pod3_dev2 = hostToEdge.Install( pod3.Get(1), pod3.Get(4));
	pod3_dev3 = hostToEdge.Install( pod3.Get(2), pod3.Get(5));
	pod3_dev4 = hostToEdge.Install( pod3.Get(3), pod3.Get(5));
	pod3_dev5 = edgeToAggr.Install( pod3.Get(4), pod3.Get(6));
	pod3_dev6 = aggrToCore.Install( pod3.Get(4), pod3.Get(7));
	pod3_dev7 = edgeToAggr.Install( pod3.Get(5), pod3.Get(6));
	pod3_dev8 = aggrToCore.Install( pod3.Get(5), pod3.Get(7));
	...
	core_dev = edgeToAggr.Install( pod0.Get(6), core.Get(0));
	core_dev2 = edgeToAggr.Install( pod1.Get(6), core.Get(0));
	core_dev3 = edgeToAggr.Install( pod2.Get(6), core.Get(0));
	core_dev4 = edgeToAggr.Install( pod3.Get(6), core.Get(0));
	
	core_dev5 = edgeToAggr.Install( pod0.Get(6), core.Get(1));
	core_dev6 = edgeToAggr.Install( pod1.Get(6), core.Get(1));
	core_dev7 = edgeToAggr.Install( pod2.Get(6), core.Get(1));
	core_dev8 = edgeToAggr.Install( pod3.Get(6), core.Get(1));
	
	core_dev9 = edgeToAggr.Install( pod0.Get(7), core.Get(2));
	core_dev10 = edgeToAggr.Install( pod1.Get(7), core.Get(2));
	core_dev11 = edgeToAggr.Install( pod2.Get(7), core.Get(2));
	core_dev12 = edgeToAggr.Install( pod3.Get(7), core.Get(2));
	
	core_dev13 = edgeToAggr.Install( pod0.Get(7), core.Get(3));
	core_dev14 = edgeToAggr.Install( pod1.Get(7), core.Get(3));
	core_dev15 = edgeToAggr.Install( pod2.Get(7), core.Get(3));
	core_dev16 = edgeToAggr.Install( pod3.Get(7), core.Get(3));
	...
	// for speical quic server and client
	PointToPointHelper endhostToEdgeLink;
	endhostToEdgeLink.SetDeviceAttribute("DataRate", StringValue(bandwidth));
	endhostToEdgeLink.SetChannelAttribute("Delay", StringValue(delay));
	// endhostToEdgeLink.SetQueueSize(StringValue(std::to_string(leafQSize) + "p"));
	// endhostToEdgeLink.SetQueue("ns3::DropTailQueue", "MaxSize",
  //                   StringValue(std::to_string(leafQSize) + "p"));

	Ipv4AddressHelper endhostAddr;

	// client-side ingress into the fat-tree
	NetDeviceContainer leftIngress = endhostToEdgeLink.Install(sim.GetLeftNode(), pod0.Get(4));
	endhostAddr.SetBase("172.16.0.0", "255.255.255.252");
	endhostAddr.Assign(leftIngress);

	// server-side egress out of the fat-tree
	NetDeviceContainer rightEgress = endhostToEdgeLink.Install(sim.GetRightNode(), pod3.Get(4));
	endhostAddr.SetBase("172.16.0.4", "255.255.255.252");
	endhostAddr.Assign(rightEgress);

	//Routing tables
	Ipv4GlobalRoutingHelper:: PopulateRoutingTables();

	//Applications
	// Only two applications will be running real QUIC protocol, and everyone else
	// would be dummy endhosts producing the 9:1 short:long traffic pattern. 
	...
	// list of host pointers
	std::vector<Ptr<Node>> hosts = {
    pod0.Get(0), pod0.Get(1), pod0.Get(2), pod0.Get(3),
    pod1.Get(0), pod1.Get(1), pod1.Get(2), pod1.Get(3),
    pod2.Get(0), pod2.Get(1), pod2.Get(2), pod2.Get(3),
    pod3.Get(0), pod3.Get(1), pod3.Get(2), pod3.Get(3),
  };

	// list of ips
  std::vector<Ipv4Address> hostIps = {
    pod0_Iface.GetAddress(0),  pod0_Iface2.GetAddress(0), pod0_Iface3.GetAddress(0), pod0_Iface4.GetAddress(0),
    pod1_Iface.GetAddress(0),  pod1_Iface2.GetAddress(0), pod1_Iface3.GetAddress(0), pod1_Iface4.GetAddress(0),
    pod2_Iface.GetAddress(0),  pod2_Iface2.GetAddress(0), pod2_Iface3.GetAddress(0), pod2_Iface4.GetAddress(0),
    pod3_Iface.GetAddress(0),  pod3_Iface2.GetAddress(0), pod3_Iface3.GetAddress(0), pod3_Iface4.GetAddress(0),
  };

	// traffic mix
	uint32_t nFlows = 300;
	double shortFrac = 0.90;
	uint64_t shortFlowBytes = 64 * 1024;
	uint64_t longFlowBytes  = 4 * 1024 * 1024;
  uint16_t basePort = 12000;
  ...
  for (uint32_t i = 0; i < hosts.size(); i++) {
		Address sinkAddr(InetSocketAddress (Ipv4Address::GetAny(), basePort+i));
    PacketSinkHelper sink("ns3::TcpSocketFactory", sinkAddr);
    ApplicationContainer app = sink.Install(hosts[i]);
    app.Start(Seconds(0.5));
    app.Stop(Seconds(endTime));
  }

	// gen random number
  Ptr<UniformRandomVariable> Picker = CreateObject<UniformRandomVariable>();

	// create flows
  for (uint32_t f = 0; f < nFlows; f++) {

		// randomly pick two endhosts and source and dest
    src = Picker->GetInteger(0, hosts.size()-1);
    dst = Picker->GetInteger(0, hosts.size()-1);
    while (dst == src) {
			dst = Picker->GetInteger(0, hosts.size() - 1);
		}

    randVal = Picker->GetValue(0.0, 1.0);
		if (randVal < shortFrac) {
			bytes = shortFlowBytes;
		}	else {
			bytes = longFlowBytes;
		}
		
		Address sinkAddr(InetSocketAddress(hostIps[dst], basePort+dst));
    BulkSendHelper sender("ns3::TcpSocketFactory", sinkAddr);
    sender.SetAttribute("MaxBytes", UintegerValue(bytes));
    sender.SetAttribute("SendSize", UintegerValue(tcpSegmentSize));

    ApplicationContainer app = sender.Install(hosts[src]);
    double start = 1.0 + Picker->GetValue(0.0, 1.0);
    app.Start(Seconds(start));
    app.Stop(Seconds(endTime));
  }
	
	//hostToEdge.EnablePcapAll ("FatTree");
	hostToEdge.EnablePcapAll("DCN_FatTree_Pcap");
